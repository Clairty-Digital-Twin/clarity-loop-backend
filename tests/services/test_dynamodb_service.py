...
class TestValidateHealthData:
    ...
    @pytest.mark.asyncio
    async def test_validate_health_data_valid(self, dynamodb_service, valid_health_data):
...
class TestPutItem:
    ...
    async def test_put_item_success(self, dynamodb_service, mock_table):
        ...
        with patch("uuid.uuid4", return_value="generated-id"):
            item_id = await dynamodb_service.put_item("test_table", item, user_id="test_user")
    ...
    @pytest.mark.asyncio
    async def test_put_item_health_data_validation(self, dynamodb_service, mock_table, valid_health_data):
