...
class DynamoDBHealthDataRepository(HealthDataRepositoryPort):
    ...
    async def save_health_data(
        ...
        try:
            ...
            raise DatabaseError(f"Failed to save health data: {str(e)}")
        ...
        try:
            ...
            if start_date and end_date:
                key_condition &= Key('sk').between(
                    f"HEALTH#{start_date.isoformat()}",
                    f"HEALTH#{end_date.isoformat()}"
                )
            elif start_date:
            ...
            raise DatabaseError(f"Failed to retrieve health data: {str(e)}")
        except Exception as e:
    ...
    async def update_processing_status(
        ...
        try:
            ...
            if analysis_results:
                ...
                expr_values[':results'] = self._serialize_item({'data': analysis_results})['data']
            
            self.table.update_item(
                Key={
                    'pk': f"USER#{user_id}",
                    'sk': f"HEALTH#{timestamp}"
                },
