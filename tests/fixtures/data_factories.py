...
class UserFactory(factory.Factory):
    ...
    date_of_birth = factory.LazyFunction(
        lambda: fake.date_of_birth(minimum_age=18, maximum_age=80).isoformat()
    )
...
def generate_activity_timeline(user_id: str, days: int = 7) -> list[dict[str, Any]]:
    ...
    for day in range(days):
        ...
        if random.random() > 0.7:
            ...
            afternoon_time = date.replace(
                hour=random.randint(12, 17), minute=random.randint(0, 59)
            )
...
def generate_test_dataset(num_users: int = 5) -> dict[str, Any]:
    ...
    for _ in range(num_users):
        ...
        metrics = generate_health_metrics_batch(
            user["user_id"], days=7, metrics_per_day=3
        )
