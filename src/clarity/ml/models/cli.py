...
from .registry import ModelRegistry, ModelRegistryConfig, ModelMetadata, ModelTier, initialize_legacy_models
from .manager import ModelManager, ModelLoadConfig, LoadingStrategy
...
@click.option('--models-dir', type=click.Path(), default='./models', help='Models directory')
...
async def init(ctx):
    ...
    console.print(f"[blue]Info:[/blue] Added legacy PAT models (PAT-S, PAT-M, PAT-L)")
...
async def list(ctx):
    ...
    grouped_models: Dict[str, List[ModelMetadata]] = {}
    for model in models:
...
async def download(ctx, model_id, version, source_url):
    ...
    if not model:
        console.print(f"[red]Error:[/red] Model {model_id}:{version} not found in registry")
        return
...
@click.option('--tier', type=click.Choice(['small', 'medium', 'large', 'quantized']), required=True, help='Model tier')
...
@click.pass_context
async def register(ctx, model_id, name, version, tier, source_url, checksum, size, description, tags):
...
async def alias(ctx, alias, model_id, version):
    ...
    if success:
        console.print(f"[green]✓[/green] Created alias '{alias}' -> {model_id}:{version}")
    else:
...
async def status(ctx):
    ...
    downloaded_models = [m for m in models if m.local_path and Path(m.local_path).exists()]
...
@click.option('--create-mocks/--no-create-mocks', default=True, help='Create mock models')
@click.pass_context
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{url}/predict", json=request_data) as response:
                if response.status == 200:
                    ...
                    console.print(f"[green]✓[/green] Prediction successful")
    ...
    try:
        while True:
            try:
                ...
                console.print(f"[{status_color}]Server Status: {health.get('status', 'unknown')}[/{status_color}]")
                console.print(f"[blue]Loaded Models: {health.get('loaded_models', 0)}[/blue]")
                console.print(f"[blue]Total Memory: {health.get('total_memory_mb', 0):.1f} MB[/blue]")
                ...
                if metrics:
                    ...
                    for model_id, model_metrics in metrics.items():
                        error_rate = model_metrics.get('error_count', 0) / max(model_metrics.get('total_inferences', 1), 1)
                        table.add_row(
                            model_id,
...
@cli.command()
@click.option('--max-size-gb', type=float, default=10.0, help='Maximum cache size in GB')
@click.pass_context
async def cleanup(ctx, max_size_gb):
    ...
    if removed_count > 0:
        ...
        console.print(f"[blue]Info:[/blue] Cache is within size limit ({max_size_gb}GB)")
