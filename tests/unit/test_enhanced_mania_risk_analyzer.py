...
class TestEnhancedManiaRiskAnalyzer:
    ...
    def create_health_metrics_with_phase_advance(self):
        ...
        for i in range(14):
            ...
            bio_data = BiometricData(heart_rate=75 if i < 7 else 65)  # Elevated HR
    ...
    def test_analyze_with_phase_advance(self):
        ...
        sleep_features = SleepFeatures(
            total_sleep_minutes=sum(s.total_sleep_minutes for s in recent_sleep)
            / len(recent_sleep),
    ...
    def test_analyze_with_variability_spike(self):
        ...
        result = self.analyzer.analyze(
            recent_health_metrics=metrics[:7], baseline_health_metrics=metrics[7:14]
        )
    ...
    def test_time_to_episode_prediction(self):
        ...
        assert (
            "Risk window:" in result.clinical_insight
            or "Monitor closely" in result.clinical_insight
        )
    ...
    def test_personal_baseline_integration(self):
        ...
        # Second analysis with extreme deviations
        sleep_features = SleepFeatures(
            total_sleep_minutes=120,  # Extreme: 2 hours vs ~8 baseline (should be >2 SD)
            sleep_efficiency=0.65,  # Poor efficiency
            consistency_score=0.2,  # Very inconsistent
            sleep_latency=5.0,  # Falls asleep too quickly (mania sign)
            awakenings_count=0.0,  # No awakenings (unusual)
        ...
        # Should detect personal deviations through higher risk score
        assert result2.risk_score > result1.risk_score
        # Should have critical sleep factors (even if baseline deviation text isn't shown)
        assert any(
            "critically low sleep" in f.lower() for f in result2.contributing_factors
        )
        # The baseline integration is working if risk scores differ
    ...
    def test_enhanced_clinical_insights(self):
        ...
        if result.alert_level == "high":
            assert (
                "days" in result.clinical_insight
                or "Risk window" in result.clinical_insight
            )
