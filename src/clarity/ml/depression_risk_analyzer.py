...
class DepressionRiskAnalyzer:
            ...
            if baseline and "avg_sleep_hours" in baseline:
                ...
                if baseline_hours > 0:
                    ...
                    if increase > 0.3:  # 30% increase
                        ...
                        factors.append(f"Sleep increased {int(increase*100)}% from baseline")
    ...
    def _analyze_activity_reduction(
        ...
        if pat_metrics:
            if "social_interaction_score" in pat_metrics:
                if pat_metrics["social_interaction_score"] < self.config.social_withdrawal_threshold:
            ...
            if "activity_fragmentation" in pat_metrics:
                if pat_metrics["activity_fragmentation"] < 0.3:  # Very low fragmentation
    ...
    def _generate_recommendations(
        ...
        if level in ["high", "moderate"]:
            ...
            if level == "high":
                recommendations.append("Schedule appointment with mental health provider")
