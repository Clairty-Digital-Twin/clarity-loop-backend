...
class ProgressStep:
    ...
    def fail(self, message: str, error: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None) -> None:
        """Mark step as failed."""
        self.status = "failed"
        self.end_time = time.time()
        self.message = message
...
class StartupProgressReporter:
    ...
    def __init__(self, output: Optional[TextIO] = None, enable_colors: bool = True) -> None:
        ...
        self.colors = {
            ...
            "cyan": "\033[36m",
            "gray": "\033[90m",
        } if self.enable_colors else {key: "" for key in ["reset", "bold", "green", "yellow", "red", "blue", "cyan", "gray"]}
    ...
    def add_step(self, name: str, phase: Optional[ProgressPhase] = None) -> ProgressStep:
    ...
    def complete_step(self, step: ProgressStep, message: str = "", details: Optional[Dict[str, Any]] = None) -> None:
        """Mark step as completed."""
    ...
    def fail_step(self, step: ProgressStep, message: str, error: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None) -> None:
        """Mark step as failed."""
        ...
        if error:
            self._print(f"    {self._colorize('Error:', 'red')} {str(error)}")
