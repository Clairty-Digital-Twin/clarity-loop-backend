...
from clarity.core.exceptions import DataValidationError, ProcessingError
from clarity.integrations.healthkit import HealthDataBatch, HealthDataType, HealthDataPoint
...
class ProcessedHealthData:
    ...
    heart_rate_series: Optional[np.ndarray] = None
    hrv_series: Optional[np.ndarray] = None
    respiratory_rate_series: Optional[np.ndarray] = None
    movement_proxy_vector: Optional[np.ndarray] = None  # For PAT model
    
    # Summary statistics
    resting_hr: Optional[float] = None
    max_hr: Optional[float] = None
    avg_hr: Optional[float] = None
    hrv_median: Optional[float] = None
    ...
    avg_temp_deviation: Optional[float] = None
    
    # VO2 Max
    vo2_max: Optional[float] = None
    vo2_max_trend: Optional[float] = None  # Change from previous
...
class AppleWatchDataProcessor:
    ...
    async def process_health_batch(
        ...
        try:
            ...
            self.logger.error(f"Error processing health batch: {str(e)}")
            raise ProcessingError(f"Failed to process health data: {str(e)}")
    ...
    async def _process_heart_rate(
        ...
        if len(values) > 1:
            ...
            minute_values = pd.Series(minute_values).fillna(method='ffill', limit=5).values
    ...
    async def _process_hrv(
        ...
        if len(values) > 1:
            ...
            minute_values = pd.Series(minute_values).fillna(method='ffill', limit=10).values
        else:
    ...
    async def _process_steps(
        ...
        self.logger.info(f"Processed {total_steps:.0f} total steps over {target_duration_days} days")
