...
class CircadianPhaseResult:
    ...
    baseline_midpoint: Optional[float]  # Baseline sleep midpoint (hours from midnight)
    current_midpoint: Optional[float]  # Current sleep midpoint
...
class CircadianPhaseDetector:
    ...
    def detect_phase_shift(
        ...
        baseline_sleep_metrics: Optional[List[HealthMetric]] = None,
        ...
        if baseline_sleep_metrics:
            ...
            if len(recent_midpoints) >= 5:
                baseline_midpoint = np.median(recent_midpoints[:len(recent_midpoints)//2])
            else:
    ...
    def _extract_sleep_midpoints(self, sleep_metrics: List[HealthMetric]) -> List[float]:
        ...
        for metric in sleep_metrics:
            if metric.sleep_data and metric.sleep_data.sleep_start and metric.sleep_data.sleep_end:
                ...
                if sleep_end < sleep_start:
                    ...
                    duration = (sleep_end + timedelta(days=1) - sleep_start).total_seconds() / 3600
                else:
    ...
    def _assess_clinical_significance(
        ...
        elif direction == "delay" and abs_shift >= self.SIGNIFICANT_DELAY_HOURS:
    ...
    def calculate_phase_variability(
        ...
        if len(phase_changes) >= 3:
            ...
            older_var = np.std(phase_changes[:-3]) if len(phase_changes) > 3 else recent_var
