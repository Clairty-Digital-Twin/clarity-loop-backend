...
from .manager import ModelManager, ModelLoadConfig, LoadingStrategy, LoadedModel
...
class ProgressiveLoadingService:
    ...
    async def get_model_status(self, model_id: str, version: str = "latest") -> Optional[ModelAvailabilityStatus]:
    ...
    async def _start_eager_loading(self):
        ...
        # Update status
        for (task, unique_id), result in zip(load_tasks, results):
...
_progressive_service: Optional[ProgressiveLoadingService] = None


async def get_progressive_service(config: Optional[ProgressiveLoadingConfig] = None) -> ProgressiveLoadingService:
...
@asynccontextmanager
async def progressive_loading_lifespan(config: Optional[ProgressiveLoadingConfig] = None):
