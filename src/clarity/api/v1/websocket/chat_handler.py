...
from clarity.services.gemini_service import GeminiService
...
class WebSocketChatHandler:
    ...
    async def _analyze_for_health_insights(self, message: ChatMessage):
        ...
        health_keywords = [
            "sleep", "tired", "energy", "stress", "anxiety", "mood",
            "exercise", "workout", "heart rate", "steps", "activity",
            "pain", "headache", "feel", "feeling", "health", "wellness"
        ]
            ...
            try:
                ...
                if insight_response.get("insights"):
                    insight_message = HealthInsightMessage(
                        ...
                        confidence=insight_response["insights"][0].get("confidence", 0.8),
                        category="conversational_health",
                        recommendations=insight_response.get("recommendations", [])
                    )
                    
                    # Send insight back to user
                    await connection_manager.send_to_user(message.user_id, insight_message)
                    
            except Exception as e:
                logger.error(f"Error generating health insight: {e}")
    
    async def process_typing_message(self, websocket: WebSocket, message: TypingMessage):
        ...
        await connection_manager.broadcast_to_room(
            "general", 
            message, 
            exclude_user=connection_info.user_id
        )
    ...
    async def trigger_health_analysis(self, user_id: str, health_data: dict):
...
async def websocket_chat_endpoint(
    ...
    try:
        ...
        username = user.display_name if user and user.display_name else f"User_{user_id[:8]}"
        ...
        logger.info(f"WebSocket chat connection established for {username} in room {room_id}")
            ...
            try:
                ...
                try:
                    ...
                    if message_type == MessageType.MESSAGE:
                        ...
                        await chat_handler.process_chat_message(websocket, message)
                    
                    elif message_type == MessageType.TYPING:
                        message = TypingMessage(**message_data)
                        await chat_handler.process_typing_message(websocket, message)
        ...
        logger.info(f"WebSocket connection closed for {username if 'username' in locals() else 'unknown user'}")
...
async def websocket_health_analysis_endpoint(
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if message_data.get("type") == "health_data":
                        ...
                        await chat_handler.trigger_health_analysis(user_id, health_data)
        ...
        await connection_manager.disconnect(websocket, "Health analysis connection closed")
...
async def get_chat_stats():
    ...
    return {
        ...
        "rooms": {
            room_id: len(users) 
            for room_id, users in connection_manager.rooms.items()
        }
    }
