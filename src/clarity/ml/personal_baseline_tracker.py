...
class PersonalBaselineTracker:
    ...
    def get_baseline(self, user_id: str) -> Optional[PersonalBaseline]:
    ...
    def _extract_sleep_data(self, metrics: List[HealthMetric]) -> List[Dict[str, float]]:
        ...
        for metric in metrics:
            if metric.sleep_data:
                ...
                point = {
                    ...
                    "duration_hours": sleep.total_sleep_minutes / 60.0 if sleep.total_sleep_minutes else 0,
                    "efficiency": sleep.sleep_efficiency if sleep.sleep_efficiency else 0,
                    "latency": sleep.time_to_sleep_minutes if sleep.time_to_sleep_minutes else 0,
                }
                
                # Calculate midpoint if times available
                if sleep.sleep_start and sleep.sleep_end:
                    midpoint = self._calculate_sleep_midpoint(sleep.sleep_start, sleep.sleep_end)
    ...
    def _extract_activity_data(self, metrics: List[HealthMetric]) -> List[Dict[str, float]]:
        ...
        for metric in metrics:
            if metric.activity_data:
                ...
                activity_by_date[date].append({
                    "steps": activity.steps if activity.steps else 0,
                    "active_energy": activity.active_energy if activity.active_energy else 0,
                    "exercise_minutes": activity.exercise_minutes if activity.exercise_minutes else 0,
                })
    ...
    def _extract_biometric_data(self, metrics: List[HealthMetric]) -> List[Dict[str, float]]:
        ...
        for metric in metrics:
            if metric.biometric_data:
                ...
                if bio.heart_rate or bio.heart_rate_variability:
                    bio_points.append({
                        "date": metric.created_at.date(),
                        "hr": bio.heart_rate if bio.heart_rate else 0,
                        "hrv": bio.heart_rate_variability if bio.heart_rate_variability else 0,
                    })
        
        return bio_points
    
    def _update_sleep_baseline(self, baseline: PersonalBaseline, sleep_data: List[Dict[str, float]]):
    ...
    def _update_activity_baseline(self, baseline: PersonalBaseline, activity_data: List[Dict[str, float]]):
        ...
        exercise = [a["exercise_minutes"] for a in activity_data if a["exercise_minutes"] >= 0]
    ...
    def _update_physiological_baseline(self, baseline: PersonalBaseline, bio_data: List[Dict[str, float]]):
    ...
    def _update_variability_baseline(
        ...
        if len(sleep_data) >= 7:
            durations = [s["duration_hours"] for s in sleep_data if s["duration_hours"] > 0]
            if durations:
                baseline.sleep_variability_baseline = np.std(durations) / np.mean(durations)
    ...
    def _update_circadian_profile(self, baseline: PersonalBaseline, sleep_data: List[Dict[str, float]]):
        ...
        if len(midpoints) >= 7:
            ...
            baseline.typical_bedtime = (baseline.sleep_midpoint_median - avg_duration/2) % 24
            baseline.typical_waketime = (baseline.sleep_midpoint_median + avg_duration/2) % 24
    ...
    def _count_unique_days(self, metrics: List[HealthMetric]) -> int:
        ...
        unique_days = set()
        for metric in metrics:
            unique_days.add(metric.created_at.date())
    ...
    def export_baseline(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "sleep": {
                ...
                "duration_iqr": [baseline.sleep_duration_p25, baseline.sleep_duration_p75],
