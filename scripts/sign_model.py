...
class ModelSigner:
    ...
    def verify_model(self, model_path: Path) -> tuple[bool, str | None]:
        ...
        try:
            ...
            if expected_size and current_size != expected_size:
                return (
                    False,
                    f"File size mismatch: expected {expected_size}, got {current_size}",
                )
        ...
        for model_path in model_files:
            try:
                ...
                if result:
                    ...
                    logger.error(
                        "✗ %s: FAILED - %s", model_path.relative_to(directory), error
                    )
...
Examples:
  ...
  python sign_model.py sign-all --models-dir models/
    ...
    # Sign-all command
    sign_all_parser = subparsers.add_parser(
        "sign-all", help="Sign all models in a directory"
    )
    ...
    verify_all_parser = subparsers.add_parser(
        "verify-all", help="Verify all models in a directory"
    )
    ...
    try:
        ...
        if args.command == "sign":
            ...
            signatures = signer.sign_directory(
                args.models_dir, recursive=not args.no_recursive
            )
            logger.info("✓ Signed %d model files", len(signatures))
