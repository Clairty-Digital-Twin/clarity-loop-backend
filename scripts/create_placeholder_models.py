...
def create_placeholder_weights(config):
    ...
    for layer_idx in range(config["num_layers"]):
        ...
        weights[f"{prefix}/multi_head_attention/key/kernel:0"] = np.random.randn(
            config["embed_dim"], config["embed_dim"]
        ).astype(np.float32)
        ...
        weights[f"{prefix}/ffn/dense_1/bias:0"] = np.random.randn(
            config["ff_dim"]
        ).astype(np.float32)
        weights[f"{prefix}/ffn/dense_2/kernel:0"] = np.random.randn(
