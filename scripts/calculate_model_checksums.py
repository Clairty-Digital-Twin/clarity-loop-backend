...
for model_file in models_dir.glob("*.h5"):
    ...
    model_size = "small" if "PAT-S" in model_file.name else ("medium" if "PAT-M" in model_file.name else "large")
    print(f'    "{model_size}": "{checksum}",  # SHA-256 of authentic {model_file.name}')