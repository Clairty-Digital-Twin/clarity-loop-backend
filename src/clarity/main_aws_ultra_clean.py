...
class PATAnalysisRequest(BaseModel):
    ...
    start_date: str
    end_date: str
    analysis_type: str = Field(default="sleep", description="Type of analysis: sleep, activity, etc")
...
class MetricRequest(BaseModel):
    ...
    aggregation: Optional[str] = Field(default="daily", description="hourly, daily, weekly")
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting CLARITY Ultra Clean backend in {ENVIRONMENT} mode")
...
app = FastAPI(
    title="CLARITY Digital Twin Platform (Ultra Clean)",
    description="Full-featured AWS-native health data backend with 35+ endpoints",
    version="3.0.0",
    lifespan=lifespan,
)
...
async def get_user_health_data(
    ...
    try:
        ...
        if current_user.get("user_id") != user_id and current_user.get("user_id") != "api-user":
...
async def upload_healthkit_data(
    ...
    try:
        # Upload to S3
        user_id = current_user.get("user_id")
        file_key = f"healthkit/{user_id}/{datetime.utcnow().isoformat()}_{file.filename}"
        ...
        logger.error(f"Failed to upload HealthKit data: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_available_models(current_user: Dict[str, Any] = Depends(get_current_user)):
