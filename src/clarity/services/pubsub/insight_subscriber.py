...
class GeminiInsightGenerator:
    ...
    async def generate_health_insight(
        ...
        try:
            ...
            self.logger.info("Generated and stored health insight for user: %s", user_id)

            return insight

        except Exception as e:
            self.logger.error("Failed to generate health insight: %s", e)
            raise
        ...
        if "summary_stats" in analysis_results:
            ...
            if "health_indicators" in summary:
                ...
                if "cardiovascular_health" in indicators:
                    ...
                    metrics_lines.append(f"- Cardiovascular Health Score: {cv.get('circadian_rhythm', 0):.2f}/1.0")

                if "respiratory_health" in indicators:
                    resp_health = indicators["respiratory_health"]
                    metrics_lines.append(f"- Respiratory Health Score: {resp_health.get('respiratory_stability', 0):.2f}/1.0")

        metrics_summary = "\\n".join(metrics_lines) if metrics_lines else "Limited health data available"

        # Create structured prompt
        prompt = f"""You are a health AI assistant analyzing a user's health data. Generate a comprehensive health insight report.
...
Generate the health insight now:"""
    ...
    def _create_analysis_summary(self, analysis_results: dict[str, Any]) -> dict[str, Any]:
        ...
        if analysis_results.get("cardio_features"):
            ...
            summary["feature_counts"]["cardio"] = len(analysis_results["cardio_features"])

        if analysis_results.get("respiratory_features"):
            summary["modalities_processed"].append("respiratory")
            summary["feature_counts"]["respiratory"] = len(analysis_results["respiratory_features"])

        if analysis_results.get("activity_embedding"):
            summary["modalities_processed"].append("activity")
            summary["feature_counts"]["activity"] = len(analysis_results["activity_embedding"])
    ...
    async def _store_insight(self, user_id: str, upload_id: str, insight: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info("Stored insight in Firestore: user=%s, upload=%s", user_id, upload_id)

        except Exception as e:
            self.logger.error("Failed to store insight in Firestore: %s", e)
            raise
...
class InsightSubscriber:
    ...
    async def process_insight_request_message(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Processing insight request for user: %s, upload: %s",
            ...
            self.logger.info("Completed insight generation for user: %s", message_data["user_id"])
            ...
            self.logger.error("Error processing insight request message: %s", e)
