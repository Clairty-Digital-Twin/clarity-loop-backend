...
class TestCognitoAuthProviderInitialization:
    ...
    def test_cognito_auth_provider_initialization(self):
        ...
        assert provider.jwks_url == "https://cognito-idp.us-east-1.amazonaws.com/us-east-1_ABC123/.well-known/jwks.json"
    ...
    def test_cognito_auth_provider_client_creation(self, mock_boto3_client):
        ...
        mock_boto3_client.assert_called_once_with("cognito-idp", region_name="us-west-2")
...
class TestTokenVerification:
    ...
    @pytest.mark.asyncio
    async def test_verify_token_success(self, mock_time, mock_b64_decode, 
                                       mock_jwk_construct, mock_get_claims, 
                                       mock_get_headers, mock_requests_get):
    ...
    @patch('clarity.auth.aws_cognito_provider.jwt.get_unverified_headers')
    @pytest.mark.asyncio
    async def test_verify_token_kid_not_found(self, mock_get_headers, mock_requests_get):
    ...
    @patch('clarity.auth.aws_cognito_provider.time.time')
    @pytest.mark.asyncio
    async def test_verify_token_signature_verification_failed(self, mock_time, 
                                                            mock_jwk_construct, 
                                                            mock_get_claims, 
                                                            mock_get_headers,
                                                            mock_requests_get):
    ...
    @patch('clarity.auth.aws_cognito_provider.time.time')
    @pytest.mark.asyncio
    async def test_verify_token_expired(self, mock_time, mock_b64_decode, 
                                       mock_jwk_construct, mock_get_claims, 
                                       mock_get_headers, mock_requests_get):
    ...
    @patch('clarity.auth.aws_cognito_provider.time.time')
    @pytest.mark.asyncio
    async def test_verify_token_wrong_audience(self, mock_time, mock_b64_decode, 
                                             mock_jwk_construct, mock_get_claims, 
                                             mock_get_headers, mock_requests_get):
    ...
    @patch('clarity.auth.aws_cognito_provider.jwt.get_unverified_headers')
    @pytest.mark.asyncio
    async def test_verify_token_jwt_error(self, mock_get_headers):
        """Test token verification with JWT error."""
        mock_get_headers.side_effect = JWTError("Invalid token format")
        
        with patch.object(self.provider, 'jwks', self.mock_jwks):
    ...
    @patch('clarity.auth.aws_cognito_provider.jwt.get_unverified_headers')
    @pytest.mark.asyncio
    async def test_verify_token_unexpected_error(self, mock_get_headers):
...
class TestUserManagement:
    ...
    async def test_get_user_unexpected_error(self):
        ...
        self.provider.cognito_client.admin_get_user.side_effect = Exception("Network error")
    ...
    async def test_delete_user_unexpected_error(self):
        ...
        self.provider.cognito_client.admin_delete_user.side_effect = Exception("Network error")
    ...
    async def test_update_user_unexpected_error(self):
        ...
        self.provider.cognito_client.admin_update_user_attributes.side_effect = Exception("Network error")
...
class TestAuthentication:
    ...
    async def test_authenticate_unexpected_error(self):
        ...
        self.provider.cognito_client.initiate_auth.side_effect = Exception("Network error")
...
class TestProductionScenarios:
    ...
    async def test_complete_user_lifecycle(self):
        ...
        with patch.object(self.provider, 'get_user', return_value=retrieved_user):
            updated_user = await self.provider.update_user("user123", display_name="Updated Name")
    ...
    async def test_provider_resilience_to_network_issues(self):
        ...
        with patch('clarity.auth.aws_cognito_provider.requests.get') as mock_get:
            ...
            with patch('clarity.auth.aws_cognito_provider.time.time', return_value=10000.0):
